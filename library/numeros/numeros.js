var numeroALetras = (function () {
  ...
  return NumeroALetras = (num, currency) => {
    currency = currency || {};
    let data = {
      numero: num,
    ...
    //if (data.centavos >= 0) {
    data.letrasCentavos = data.centavos.toString().trim().padStart(2, '0') + '/100 M.N.';
    ...
    if (data.enteros == 1) {
      result = Millones(data.enteros) + ' ' + data.letrasMonedaSingular + ' ' + data.letrasCentavos;
    } else {
      result = Millones(data.enteros) + ' ' + data.letrasMonedaPlural + ' ' + data.letrasCentavos;
    };
    return '==' + result + '=='
  };
