...
const numeroALetras = require('../numeros/numeros');
...
const contratoServiciosPdf = (contratoServiciosBD) => {



  console.log('1.- EN contratoServiciosPdf->', contratoServiciosBD);

  let cs = contratoServiciosBD;
  //console.log('EN contratoServiciosPdf->', cs);
  ...
  var doc = new pdf({
    ...
    margin: 0,
    info: {
      Title: 'CONTRATO DE PRESTACIÓN DE SERVICIOS HOSPITALARIOS: msi-00 v1.0',
  ...
  let imgFormato = path.resolve(__dirname, '../../msiformatos/msi00.jpg');
  ...
  let pathCfg = path.join(__dirname, './contratoServiciosFields.cfg');
  ...
  ////imgFormato = path.resolve(__dirname, '../../msiformatos/msi20-2.jpg');
  ...
  const fileName = 'MSI00-' + Date.now() + '.pdf';
...
module.exports = contratoServiciosPdf;
