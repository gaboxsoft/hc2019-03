...
const historiaClinicaPdf = (paciente) => {
  ...
  //});
    pages[0].forEach(function (field) {
      writeLine(doc, eval(field.name), field.row, field.col, field.align, field.fontSize, field.color, field.width, field.indent);
    });
  ...
  pages[1].forEach(function (field) {
    writeLine(doc, eval(field.name), field.row, field.col, field.align, field.fontSize, field.color, field.width, field.indet);
  });
  ...
  if (paciente.firmaBase64HC) {
    var dataImgDecodeFromBase64 = new Buffer.from(paciente.firmaBase64HC, 'base64');
    //doc.image(dataImgDecodeFromBase64, 275, doc.y + 20, { width: 300 })
    doc.image(dataImgDecodeFromBase64, 275, altoHoja-120 + 20, { width: 300 })
  }
...
function writeLine(doc, text, row, col, align, fontSize, color, width, indent) {
  const vacio = '';
